...
\begin{defn}
If $Y \rightarrow X$ is a morphism of presheaves on $\naff$, then we say $Y \rightarrow X$ is totally split if there exists a finite set $I$ and an isomorphism $f: Y \rightarrow \coprod_{I}X$.
\end{defn}

We say that a morphism of split morphisms(schemes?) is split if it is defined by a set map.

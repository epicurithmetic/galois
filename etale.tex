...
\begin{defn}
Let $Y \rightarrow X$ be a morphism of presheaves on $\naff$. If there exists an fpqc cover $(U_{i}\rightarrow X)_{i\in I}$ and a family of finite sets $N_{i}$ such that $Y \times_{X} U_{i} \cong \coprod_{I} U_{i}$, then we say the morphism $Y \rightarrow X$ is finite \'{e}tale.
\end{defn}
